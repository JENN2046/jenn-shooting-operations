...
const EXPECTED_GATE_BINDINGS = new Map(Object.entries({
  ...
  "PRE_CUTOVER_ORPHAN_CLEANUP_CONTROL": {
    "status": "BLOCKED",
    "evidence": "REPOSITORY_ALL_ENTRY_POINT_DISABLE_DRAIN_IMPLEMENTED_DEPLOYMENT_ACCEPTANCE_PENDING"
  },
  ...
  "PRODUCTION_ATTACHMENT_COPY_CAPABILITY": {
    "status": "BLOCKED",
    "evidence": "REPOSITORY_ATTACHMENT_COPY_PARITY_ENGINE_IMPLEMENTED_LIVE_QUIESCENCE_PROVIDER_AND_TARGET_VOLUME_ACCEPTANCE_PENDING"
  },
  ...
  "RESTORED_CLEANUP_DISABLE_CAPABILITY": {
    "status": "BLOCKED",
    "evidence": "REPOSITORY_DISABLE_DRAIN_IMPLEMENTED_BUT_EXACT_PROD_14_SCHEDULE_CANCELLATION_ROLLBACK_NOT_IMPLEMENTED"
  },
...
const EXPECTED_ACTION_BINDINGS = new Map(Object.entries({
  ...
  "PROD-13-CUTOVER-SWITCH": {
    ...
    "sideEffect": "IRREVERSIBLE_OR_EXTERNAL",
    "status": "BLOCKED_PREREQUISITE",
    "authorityTarget": "Exact approved source and target service database/upload write barriers and production route/data/client switch only",
    "preconditions": [
      "WO06C_VCP_EXTERNAL",
      "WO06C_KIOSK_DEVICE",
      "PRODUCTION_TARGET_FACTS",
