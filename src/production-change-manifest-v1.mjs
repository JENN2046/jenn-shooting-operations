...
const EXPECTED_GATE_BINDINGS = new Map(Object.entries({
  ...
  "PRE_CUTOVER_ORPHAN_CLEANUP_CONTROL": {
    "status": "BLOCKED",
    "evidence": "STARTUP_AND_PERIODIC_ORPHAN_CLEANUP_DISABLE_NOT_IMPLEMENTED"
  },
...
const EXPECTED_ACTION_BINDINGS = new Map(Object.entries({
  ...
  "PROD-05-START-ISOLATED-CONTAINER": {
    ...
    "effects": [
      "Start new service bound to host loopback only with startup and periodic orphan-upload cleanup disabled through cutover"
    ],
    "rollbackActionIds": [
      "ROLLBACK-02-STOP-NEW-CONTAINER"
    ],
...
const FORBIDDEN_SECRET_PATTERNS = Object.freeze([
  ...
  /(?:"(?:access_token|VIEWER_TOKEN|SUBMITTER_TOKEN|SCHEDULER_TOKEN|ADMIN_TOKEN)"|'(?:access_token|VIEWER_TOKEN|SUBMITTER_TOKEN|SCHEDULER_TOKEN|ADMIN_TOKEN)'|(?:access_token|VIEWER_TOKEN|SUBMITTER_TOKEN|SCHEDULER_TOKEN|ADMIN_TOKEN))\s*[:=]\s*(?:"[^"]{8,}"|'[^']{8,}'|[^\s"',;]{8,})/iu,
  /sk-[A-Za-z0-9_-]{16,}/u,
