...
The persistent target fence initially denies **every ordinary writer** and has no synchronization exception while it is acquired and while the initial all-process database and attachment drain completes. The offline path never creates an exception. The synchronized path may create exactly one narrow exception only after the exact source-wide database/upload barrier is held and all source work is drained.
