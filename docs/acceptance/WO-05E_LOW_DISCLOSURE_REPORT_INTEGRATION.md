...
- stable exclusion-code allowlist, order, uniqueness and dataset-count bounds; Level-A-disqualifying exclusion counts are bounded by `eligibilityCounts.ineligible`; Level-B snapshot exclusions are bounded by the `levelA - levelB` shortfall; Level-C shadow exclusions are bounded by the `levelB - levelC` shortfall; outcome exclusions preserve their mutual-exclusion bound; `EVENT_AFTER_CUTOFF` is a subset of `EVENT_OUTSIDE_DATASET_WINDOW`;
