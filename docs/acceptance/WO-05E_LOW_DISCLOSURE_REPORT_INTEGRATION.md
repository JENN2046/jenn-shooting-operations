...
- stable exclusion-code allowlist, order, uniqueness and dataset-count bounds; non-window Level-A classifier exclusion counts are bounded by the processed-ineligible cohort `eligibilityCounts.ineligible - EVENT_OUTSIDE_DATASET_WINDOW`, while window/cutoff exclusions are validated separately; Level-B snapshot exclusions are bounded by the `levelA - levelB` shortfall and by the processed cohort (`total - EVENT_OUTSIDE_DATASET_WINDOW`); Level-C shadow/outcome exclusions are likewise bounded by the processed cohort and the `levelB - levelC` shortfall; outcome exclusions preserve their mutual-exclusion bound; `EVENT_AFTER_CUTOFF` is a subset of `EVENT_OUTSIDE_DATASET_WINDOW`;
